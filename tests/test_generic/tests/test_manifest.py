...
class ManifestTest(ManifestLinter, IndustryCase):
    ...
    def _validate_manifest(self, module, manifest_data):
        ...
        for key, expected_value in MANDATORY_KEYS.items():
            ...
            if key == 'category':
                ...
                self.assertTrue(all(val.startswith(f'{key}/') for val in value), f"Files must be in '{key}/' for {module}")
        
        # Validate files in folders
        for folder in ['data', 'demo']:
            self._test_files_in_manifest(module, manifest_data, folder)

        #check for the assets files
    ...
    def _test_files_in_manifest(self, module, manifest_data, folder_name):
        ...
        if data_folder.exists():
            data_files = {str(file.relative_to(data_folder.parent)) for file in data_folder.glob('*') if file.is_file()}
            manifest_files = set(manifest_data.get(folder_name, []))

            self.assertFalse(
                data_files - manifest_files,
                f"Files in folder not listed in manifest {folder_name} for {module}"
            )
            self.assertFalse(
                manifest_files - data_files,
                f"Files listed in manifest {folder_name} not found in folder for {module}"
            )
